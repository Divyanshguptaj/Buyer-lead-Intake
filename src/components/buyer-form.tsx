...
import { IndianLocationInput, indianStates } from '@/components/ui/indian-city-input';
...
export function BuyerForm({ initialData, onSubmit, isSubmitting }: BuyerFormProps) {
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div>
            <label htmlFor="bhk" className="block text-sm font-medium text-gray-700 mb-1">
              BHK {(['Apartment', 'Villa'].includes(selectedPropertyType)) ? '*' : ''}
            </label>
            <select
              id="bhk"
              {...register('bhk')}
              disabled={!['Apartment', 'Villa'].includes(selectedPropertyType)}
        ...
        <div className="mt-6">
          ...
          <p className="text-xs text-gray-500 mb-1">
            Tags help categorize leads. Color-coded for easy identification. At least one tag is required.
          </p>
          <TagInput 
            value={tags}
            onChange={(newTags) => {
            ...
            suggestions={[
              // Priority tags
              "Hot Lead", "Urgent", "VIP", "Premium", "High Priority",
              // Client type
              "Investor", "End User", "Cash Buyer", "First Time", "Repeat Client",
              // Budget tags
              "High Budget", "Mid Budget", "Low Budget", "Negotiable",
              // Status tags
              "Follow Up", "Viewed", "Interested", "Not Interested", "Closed",
              // Property specific
              "Ready to Move", "Under Construction", "Off Plan", "Resale"
            ]}
            required
            minTags={1}
